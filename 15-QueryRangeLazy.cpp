...
ll queryLazy(ll s,ll e,ll l,ll r,ll *tree,ll index){
    if(lazy[index] != 0){
        tree[index] += lazy[index];
        if(s != e){
            lazy[2*index] += lazy[index];
    ...
    ll middle =(s + e)/2;
    ll left = queryLazy(s,middle,l,r,tree,2*index);
    ll right = queryLazy(middle+1,e,l,r,tree,2*index+1);
...
int main() {
    ...
    cout<<queryLazy(0,n-1,1,4,tree,1);
}
